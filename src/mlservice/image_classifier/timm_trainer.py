...
class Trainer(object):
    ...
    def __init__(self, dataset_folder='./datasets/dataset_splits',
        ...
        if model_dir is not None:
            ...
            self.chkpt_dir = os.path.join(chkpt_dir, model_name + '_' + datetime.datetime.now().strftime(
                '%y%m%s%H%M%S'))
    ...
    def train(self, time_limit=None, n_epochs=20):
        ...
        for epoch in range(n_epochs):
            ...
            for batch in (pbar := tqdm(self.train_loader)):
                self.lr_schedule.step(curr_iter / n_iters)
                curr_iter += 1
