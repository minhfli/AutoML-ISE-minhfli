...
class AutogluonTrainer(object):
	...
	def train(self, label: str, train_data_path: Path, val_data_path: Path, model_path: Path) -> Union[TabularPredictor, None]:
		try:
			...
			if self.has_special_types:
				...
				feature_metadata = feature_metadata.add_special_types({col: ['image_path'] for col in self.data_args['image_cols']})
				feature_metadata = feature_metadata.add_special_types({col: ['text'] for col in self.data_args['text_cols']})
	...
	@staticmethod
	def evaluate(predictor: TabularPredictor, test_data_path: Path,
				 metrics: str = "accuracy") -> Optional[float]:

		try:
			test_acc = predictor.evaluate(test_data_path).get("accuracy")
			return test_acc
		except Exception as e:
...
if __name__ == "__main__":
	...
	train_path = "/home/minhduck/Documents/projects/AutoML-Platform/src/mlservice/app/test/titanic/train.csv"
	trainer = AutogluonTrainer()
	p=trainer.train('Survived', train_path, None, '../test/trained_models/ag_%s'%str(datetime.now().strftime("%y%m%d_%H%M%S")))
	print(AutogluonTrainer.evaluate(p, train_path))